...
public class AddCar extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag1){
			...
			boolean flag2=service.insertCar(car);
			if(flag2){
				 out.print("<script>alert('添加成功！返回显示页面');window.location.href='SelectAllCar'</script>");
			}else{
				out.print("<script >alert('数据冲突！添加失败');window.location.href='AddCar.jsp'</script>");
			}
		}
		else{
			out.print("<script>alert('数据错误！添加失败');window.location.href='AddCar.jsp'</script>");
		}
	}
